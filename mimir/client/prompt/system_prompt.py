...
_SECTION_VALIDATION = (
    ...
    "When the code you changed is registered with an available tool that builds and runs it "
    "(it declares its own build command), let that tool build: building it by hand first builds "
    "the same target twice. Otherwise use the build system the project ships (make, CMake, "
    "ninja...), not the compiler called by hand. "
    "Build only the targets your change reaches: the library or executable holding the edited files, "
    "and what you will run in step 3. Never build `all` when one target is enough, and never clean "
    "first: an incremental build recompiles only what changed. "
    "Build in parallel, one job per core the platform/environment query tool reports (`make -jN`, "
    "`cmake --build <dir> -j N`; ninja is parallel already).\n"
    ...
    "then quantify the speedup with timed runs, consulting the platform and HPC advisors "
    "for architecture-aware choices. An optimization without a measurement is a claim, and a faster "
    "wrong answer is a regression.\n"
    "- Optimize for the machine the code will RUN on. From a login node that is a compute node: the "
    "one the user named, else the one the code calls for; ask if unclear. Compile for its profile "
    "(its `-march`, never `native` elsewhere) and time every run there, baseline included — an "
    "allocation for this is in proportion. On the node itself, or with no scheduler, the host is it.\n"
    "\n"
