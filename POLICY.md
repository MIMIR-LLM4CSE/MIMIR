...
- validation runs through the `bash` server, not a dedicated validator tool. `observations._observe_bash_validation` drives it status-agnostically, with **three** outcomes rather than two, because exit 0 does not mean the same thing for every check. **Checkers** (`py_compile` → `syntax`; `ruff`/`mypy`/`pyflakes`/`black` and the compilers `gcc`/`g++`/`gfortran`/`nvcc`/`javac` → `static`) **succeed** (`status="ok"`, bash's exit 0) → recorded in `validated_files`: their output is a list of problems and an empty one is the finding, so there is nothing left for anyone to judge. **Executions** (`pytest`, `python solver.py`, `./solver`, `ctest`) exiting 0 prove only that the program reached its end — the file stays pending and the run is parked in `unjudged_runs` until the model states a verdict (see below). **Failures** (`status="error"`) increment `validation_fail_count_by_file` and return the workflow to `edit`, unchanged. A leading `cd` rebases relative operands so the resolved path matches the dirty path exactly. **Whole project (big refactors):** a green run of a recognised project validator that names no specific file — bare `pytest`, `ruff check .`, `mypy src/` or `ctest` = `_PROJECT_VALIDATORS` — covers **every** pending file at once (clearing them for a checker, parking them all behind one verdict for an execution). That set is kept to validators that are actually declared (bash `_ALLOWED_COMMANDS` + `bash_classify._EXEC_COMMANDS`); ambiguous build drivers (`make`/`cmake`) and program runs are excluded from the whole-project shortcut, bearing only on the file they explicitly name. Test files go into `tests_run` either way (feeds the regression nudge). A plugin server's dedicated `VALIDATE` tool contributes the same way via `_observe_validation_tool`.
- **Exit 0 no longer validates an execution; the model's verdict does.** Exit 0 says a program ended, never that its answer is right, and nothing downstream can read what it printed — no parser generalises across fields, convergence tables, plots, logs and physical units. So mimir never parses the *program's* output for a pass/fail; it parses the model's own statement about it, which is a format the model controls (`guardrails/verdict.py`, read from every assistant message in `_run_agent_loop`, from `content` and never the thinking block). The line is `verdict: pass|fail|unknown — <what in the output shows it>`; the last one in a message wins.
  - `pass` → `_mark_file_validated` at the run's own tier — the ordinary entry point, so the red→green promotion, the fail-count reset and the conclude transition all happen exactly as before.
  - `fail` → `_register_validation_failure(..., arms_red_green=False)`: the *existing* repair ladder, with no second mechanism. The opt-out matters — a self-declared failure followed by a self-declared pass would otherwise forge discrimination, the one signal an exit code cannot fake. **A model may lower its own credit, never raise it.**
  - `unknown` → settles the run, validates nothing, and is an acceptable ending when explained. Silence is not.
  - **How far one statement reaches follows the same asymmetry.** `fail` and `unknown` settle every outstanding run at once — withholding credit from a run the statement did not mean is never the unsafe direction, and costs a re-judgement at worst. A `pass` settles them all too *when they bear on the same files* (nothing to disambiguate: whichever run it meant, it meant that file). When they bear on **different** files, an unqualified `pass` would credit runs the model may never have read, so it settles **nothing**: the commands it could not choose between go to `verdict_scope_required`, and the nudge reads them back. The model then names its run — `verdict[<command or file>]: pass — …`, matched as a case-insensitive substring against the command and its paths — which settles exactly that one and leaves the rest open. A scope naming nothing outstanding settles nothing either, which beats guessing.
  - **What a run's output speaks for** is `observations._attributed_run_paths`. A run naming dirty files speaks for those. Naming none, it falls back to the whole pending set, because a run routinely exercises code it does not name — `pytest`, `./solver` and `python tests/test_solver.py` all validate the source behind them, and that fallback is what lets one suite settle a refactor. The exception: a run whose every named file resolves **outside** the workspace (`python /tmp/<scratch>/probe.py`, or a bare `/tmp/<scratch>/probe` naming its artifact in the command head) did name what it ran, and it was not the user's code. It is registered with no paths — a verdict is still owed on it, it just credits nothing. Without that carve-out one `verdict: pass` on a throwaway probe would validate source the probe never touched, and the prompt sends **every** throwaway probe to the scratchpad, so it is the common path rather than a corner case.
  - Recorded in `verdict_by_file` (retracted on re-edit, like the tier) and, for failures, appended to `verdict_attempts_by_file`, which deliberately **survives** re-edits: "what was tried and why it did not work" is exactly the history a re-edit would erase, and it is what the budget-exhausted hand-back reports.
  - The demand is **not** gated on `dirty_written_files`. An analysis-only session — "does the suite pass?", "why does this blow up?" — is precisely the one whose whole answer rests on a run's output.
  - **Execution tools** owe a verdict too, not just bash: `_observe_tool_run` fires for any tool carrying `CODE_EXEC` that does **not** declare a `command_prefix` scope (`proxy_exec`, `proxy_eval`, the `benchmark_*` family), attributing through the tool's declared path args, by the same `_attributed_run_paths` rule. The split is by *surface*: a shell tool's calls differ in kind call by call, so only the command text can decide; a structured tool's call *is* the execution. Mutually exclusive by construction, so no run is registered twice.
- **A run's declared verdict outranks its exit code, one way only.** A check that evaluates its own criteria, prints that they were not met and returns 0 anyway is a green exit over a red result — observed in the wild: a self-written boundary test reported "significant reflection may be present", exited 0, and was recorded as `validated: executed`. So a `check=fail` (or `verdict=fail`) line in stdout — same strict whole-line `key=value` grammar as the invariants, `numerics.observed_failure_verdict` — demotes a green run to a failure and pre-fills the model's verdict, so it is not asked to judge output that already carries its own answer. A `check=pass` line never rescues a red one.
- **validation strength is graded, not boolean.** `validated_files` answers "was it checked?"; `validation_tier_by_file` answers "with what?", on the ladder `syntax` < `static` < `executed` < `oracle` (`context/execution_context.VALIDATION_TIERS`). The tier comes from the command head. It is raised monotonically and retracted whenever `validated_files` is (a re-edit or a failed check), since evidence is about one revision of a file.
...
Appended up to `NUDGE_MAX_OUTPUT_VERDICT = 2` times when a run's output has not been judged, or when a stated `unknown` is still standing on a pending file. A reality check of the plainest kind — a program was run and nothing was said about what it printed.

Every nudge fires only on a turn with no tool calls (`maybe_append_nudge` has a single call site), so this asks at the moment the model tries to *stop*, never mid-loop. The counter is re-armed once a verdict actually settles the pending runs, so a later unjudged run earns fresh reminders — the cap is anti-spam, not a mute button. An `unknown` verdict does not re-arm it: the condition it was spent on is still open.

Three message variants: name the commands left unjudged, tell a refused broad `pass` what it must choose between (`verdict_scope_required` — checked first, since the model *did* speak and repeating "you never said what its output showed" would be false), or push a standing `unknown` towards resolution (find the reference, the documented value, the analytical limit, the invariant, the refinement trend — then re-judge, or say explicitly why it is out of reach). Three acceptable endings, one unacceptable: silence.

The guidance `validation` nudge defers to it — a file whose check already ran green is pending for want of a *judgement*, not a *check*, and "run `py_compile` again" would be wrong advice. `_pending_needing_a_check` makes that split explicit rather than relying on the layer ordering alone.
...
```
<!--mimir:ledger status="warn" files="2" summary="2 files · 1 not validated · 1 run unjudged · 2 steps open"-->
Verification ledger — machine-recorded, not model-authored:
- `solver.py` — validated: executed · verdict: pass — the L2 error is 3e-4 against the
  analytic solution
- `boundary.py` — ran, **not validated**: its output was never judged
