...
- **There is no "declared but never written" row**, and the absence is deliberate. It used to be here, derived free from `declared_edit_set` — paths scraped out of the checklist's own step text. Every other row reports what a checker, a run or the disk established; this one reported a gap against prose the model wrote earlier, and at completion time a plan it revised on purpose (two files dropped because the work turned out not to need them) is indistinguishable from two files skipped. It charged the honest revision as a gap, and inflated residual risk to `medium` for it. `unwritten_declared_files` still exists and is still read **while the work is going** — the edit→validate transition, the empty-turn corrective, the missing-evidence text — where "named earlier, not written yet" is useful guidance rather than a verdict. It matches by **resolved path**, not by string: a write records the path the file tools took (absolute outside the workspace, root-relative inside) while the prose names the file however it reads best, so a bare mention (`write wave_solver_2d.py in ../other/`) matches on basename and a relative one is resolved against the root.
- The declared set is **replaced** by each new checklist, never accumulated — `todo_write` replaces the whole list of steps, so the declaration derived from it has to as well. Accumulating meant a declaration could never be retracted: a revised plan stayed on the hook for every file any earlier version had named, which is what made the removed row misfire and what kept the edit→validate transition waiting on writes that were never coming.
